...
NewsNook 的阅读链路是 **backendless** 的：无自建推荐后端、不上传阅读画像。可选的云同步只同步订阅与配置，阅读记录不在同步域内，因此推荐画像永远只留在本机。
...
| 何时出现 | 预设候选池内累计已读 + 稍后读条目数达到 `RECOMMEND_MIN_SCOPED_DOCS`（阈值收在 `lib/recommend.ts`，UI 经 `recommendationReadiness` 拿进度，不自行硬编码数值）；未达标时轨道与以前完全一样 |
| 总开关 | 「我的 → 分类与信源」顶部的「推荐」栏开关（`recommendEnabled`，默认开启）：关闭后**所有预设**都不显示推荐栏，普通分类不受影响；重新打开且阅读仍达标时推荐栏自动恢复到第一位，依旧不抢焦点 |
| 进度反馈 | 同一张设置卡展示状态：已关闭 / 积累中（当前预设已读 X / 需 Y 篇）/ 已出现；X 由达标即停的计数给出，封顶为 Y |
...
| 何时重算 | **进入**「推荐」分类时构建一次画像；停留期间 **下拉刷新 / 顶栏刷新即「重算推荐」**——基于最新已读与稍后读重建画像、拉新候选并立即重排，无需重启应用。画像不因单次已读立刻重算（避免读完返回列表跳动）。画像不落盘、每次即时计算，因此**没有**需要单独「重置」的隐藏画像状态；已读记录本身绝不因重算被清除 |
| 空态 | 推荐栏打开但列表为空（候选都已读，或尚未取到列表）时，给专属中文空态并提示下拉刷新重算 |
| 条数上限 | 最多 **120** 条 |
...
### 3.2 就绪（`recommendationReadiness` / `isRecommendationReady`）

统计能归属到候选池信源的**去重**条目数（稍后读 + 已读），达到 `RECOMMEND_MIN_SCOPED_DOCS`（当前 **5**）即亮起；空候选池永远不亮。阈值只存在于 `lib/recommend.ts`。

`recommendationReadiness` 返回 `{ ready, scopedDocs, requiredDocs }`：`scopedDocs` 在达到阈值后提前停止累计（保持 O(阅读量) 上限，因此封顶为 `requiredDocs`，不代表池内阅读总量），设置页用它渲染「已读 X / 需 Y」。`isRecommendationReady` 是其布尔捷径。

**为什么是 5**：每篇标题贡献约 10~20 个 bigram 词面，5 篇是画像不被单篇偶读主导的下限，同时一次阅读会话内即可解锁、功能可被自然发现；再调低会放大噪声，调高只是推迟出现——排序有冷启动时间序兜底，低阈值的下行有限。
...
| 持久化 | 推荐不写入 `categoryOrder` / `hiddenCategoryIds` / 预设快照；显隐由运行时判定：`recommendEnabled` 开关（见下）与就绪判定同时满足才亮 |
| 总开关 | `Preferences.recommendEnabled`（默认 `true`；setter `setRecommendEnabled`）：全局粒度，不进预设快照；归一化时缺字段 / 脏值回落为开启，旧备份导入自然兼容 |
...
手动：新装（零阅读）任一预设都看不到推荐栏；在预设内读满阈值条数后，分类轨最前出现「推荐」，当前选中分类不跳变；点进推荐可见按已读习惯排序的未读条目；在推荐栏内下拉刷新，列表按最新已读记录立即重排；切换预设后焦点回到第一个普通分类；「我的 → 分类与信源」顶部关闭「推荐」栏后所有预设的推荐栏立即消失（若正停留在推荐栏则回到第一个普通分类），重新打开后就绪即恢复且不自动跳转；设置卡在未达标时显示「已读 X / 需 Y 篇」；新建/编辑分类时把名称或短名填成「推荐」会被拦截并提示。

## 7. 调参与风险（实现者须知）

- 阈值 `RECOMMEND_MIN_SCOPED_DOCS=5`（取值理由见 §3.2）、权重（0.55 / 0.2 / 0.25）、打散 0.82、τ=36h、limit=120、画像 cap=200 均为保守缺省，**未经大规模用户调参**；改常量请同步本文与测试。
