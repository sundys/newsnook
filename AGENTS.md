...
本地优先的 **Android / Web 新闻阅读客户端**：不登录即可完整使用；可选账户与云同步增强跨设备体验；主新闻/RSS 信息流不依赖平台式推荐算法。订阅源由用户配置，列表与正文由客户端直连上游，在应用内阅读。独立站点工作区（如知乎）可在站点边界内对上游候选做本地个性化、去重与多样性重排，但不得反向污染主新闻/RSS 状态轴。定位是**工具**，不是内容平台。
...
- 普通新闻/RSS 主路径不要引入平台式「热度排序强推」或服务端推荐依赖，也不要把正文/离线缓存上云做成阅读必经路径。知乎等独立站点工作区可在其领域边界内对上游候选做本地推荐协调，但必须本地优先、账号隔离、可退化，不能影响主新闻/RSS。
